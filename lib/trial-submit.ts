...
export async function handleTrialSubmit(request: Request) {
  ...
  if (!captchaResult.ok) {
    return NextResponse.json(
      {
        message: captchaResult.message || "验证码校验失败"
      },
      { status: 400 }
    );
  }
  ...
  if (parsed.data.website) {
    // Honeypot matched: fake success to reduce bot retries.
  ...
  return NextResponse.json({ message: "提交成功。我们将尽快安排对接。" });
}
